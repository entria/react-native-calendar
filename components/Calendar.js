...
export default class Calendar extends Component {
 ...
 prepareEventDates(eventDates, events) {
  ...
  // Render the day acronyms (S, M, etc)
  renderHeading() {
    const headings = [];
    for (let i = 0; i < 7; i++) {
  ...
  renderTimeline = () => {
    ...
    while(hour++ < 18) {
      times.push(
        <View
          key={hour}
          style={styles2.timeContainer}
        >
          <Text style={styles2.time}>
            {`0${hour}`.slice(-2)}:00
          </Text>

          <View style={styles2.timeSeparator} />
        </View>
      );
    }
...
// height: ((527 / 10) * ((10-9) + (30/60))) - 8
  ...
  render() {
    ...
    const calendarDates = this.getMonthStack(currentMonthMoment);
    const eventDatesMap = this.prepareEventDates(eventDates, events);
    const numberOfWeeks = getNumberOfWeeks(currentMonthMoment, weekStart);

    return (
      <ScrollView contentContainerStyle={[styles.calendarContainer, customStyle.calendarContainer]}>
        {this.renderTopBar()}
        ...
        {scrollEnabled ?
          (
            <ScrollView
              ...
              style={{
                height: rowHeight ? rowHeight * numberOfWeeks : null,
              }}
            >
              {calendarDates.map(date => this.renderMonthView(moment(date), eventDatesMap))}
            </ScrollView>
          ) : (
            <View ref={ref => this._calendar = ref}>
              {calendarDates.map((date) => this.renderMonthView(moment(date), eventDatesMap))}
            </View>
          )
        }
